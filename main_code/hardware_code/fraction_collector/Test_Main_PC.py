...
class SeverComm:
    ...
    def send_data(self, data_type, data_out="None"):  # First entry is data type, second entry is data
        if len(data_out) < self.buffer_size-20:
            if isinstance(data_out, str):
                msg = data_type + ',' + data_out
                self.conn.send(bytes(msg, mess_format))
        else:
            exit("The data that is being sent is to long. Adjust message or buffer(both in client and server)")
...
if __name__ == '__main__':     # Program starts from here
    ...
    FC_connection.send_data("Layout", FC_layout)  # First entry is data type, second entry is data
