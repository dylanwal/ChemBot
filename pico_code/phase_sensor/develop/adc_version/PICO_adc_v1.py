...
def main_loop():
    ...
    while True:  # infinite loop
        ...
        if poll_results:
            ...
            if message[0] == "s":
                single_measurement(message, ADC, leds)
            elif message[0] == "c":
                continuous_measurement(message, ADC, leds, poll_obj)
            elif message[0] == "g":
                ADC.gain = int(message[1:3])
            elif message[0] == "o":
                DAC.write(int(message[1:3]))
            # elif message[0] == "s":
            #     sleep(message)
            elif message[0] == "r":
                reset()
...
def single_measurement(message, ADC, leds):
    """
    ...
    """
    # process message
    pin = int(message[1:3])  # adc pins
    mode = message[3]  # 0=single or 1=differential

    # take data
    leds.value(0)  # on (flipped from expected)
    sleep(0.01)  # give a sec for led to turn on
    value = ADC.read(pin, differential=mode)
    leds.value(1)  # off (flipped from expected)

    # send data
    sys.stdout.write(struct.pack("h", value))


def continuous_measurement(message, ADC, leds, poll_obj):
    """
    ...
    for i in range(4):
        if len(message) > 3:
            pins.append(int(message[1:3]))  # adc pins
            modes.append(int(message[3]))  # 0=single or 1=differential

    # setup measurement
    leds.value(0)  # on (flipped from expected)
    sleep(0.01)  # give a sec for led to turn on
    data = array.array("h", [0] * len(pins))
    loop_tuple = tuple(range(len(pins)))
    ...
    while True:
        ...
        for i in loop_tuple:
            data[i] = ADC.read(pins[i], differential=modes[i])

        sys.stdout.write(data.tobytes())
