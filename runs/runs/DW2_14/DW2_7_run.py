...
def steady_state(profile: DynamicProfile):
    mon_vol = float(profile.flow_rate_mon[0]) * Unit("ml") * 7 * 6  # (3*7min) * flowrate(ml/min) = ml|7min residence time
    cat_vol = float(profile.flow_rate_cat[0]) * Unit("ml") * 7 * 6
    fl_vol = float(profile.flow_rate_fl[0]) * Unit("ml") * 7 * 6
    dmso_vol = float(profile.flow_rate_dmso[0]) * Unit("ml") * 7 * 6
