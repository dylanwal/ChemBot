...
def main():
    ...
    flow_rate_mon = flow_rate_total/2*0.7
    flow_rate_ctabtpa = (flow_rate_total.to("ml/min").v/2*0.3*cta_ratio) * Unit("ml/min")
    flow_rate_ctapy = (flow_rate_total.to("ml/min").v/2*0.3*(1-cta_ratio)) * Unit("ml/min")
    ...
    print("mon volume needed:", np.trapz(y=flow_rate_mon.to("ml/min").v, x=t), "ml")
    print("ctabtpa  volume needed:", np.trapz(y=flow_rate_ctabtpa.to("ml/min").v, x=t), "ml")
    print("ctapy volume needed:", np.trapz(y=flow_rate_ctapy.to("ml/min").v, x=t), "ml")




    fig = go.Figure()
    fig.add_trace(go.Scatter(x=t, y=flow_rate_ctapy.to("ml/min").v, name="flow_rate_ctapy"))
    fig.add_trace(go.Scatter(x=t, y=flow_rate_mon.to("ml/min").v, name="flow_rate_mon"))
    fig.add_trace(go.Scatter(x=t, y=flow_rate_ctabtpa.to("ml/min").v, name="flow_rate_ctabtpa"))
    ...
    fig.add_trace(go.Scatter(x=t, y=flow_rate_ctabtpa.to("ml/min").v + flow_rate_ctapy.to("ml/min").v, name="cta_check"))
    fig.add_trace(
        go.Scatter(x=t, y=(flow_rate_ctabtpa.to("ml/min").v + flow_rate_ctapy.to("ml/min").v)/flow_rate_mon.to("ml/min").v, name="cta_check"))
    fig.add_trace(
        go.Scatter(x=t, y=flow_rate_ctabtpa.to("ml/min").v + flow_rate_ctapy.to("ml/min").v+flow_rate_mon.to("ml/min").v, name="m_check"))
    ...
    # breaks
    breaks = [0, get_index(t, 100), get_index(t, 200), get_index(t, 325), -1]
    print()
    print(breaks)
    print(t[breaks])
    print("index", "mon", "ctabtpa", "ctapy", "fl")
    for i in range(1, len(breaks)):
        print(i,
              np.trapz(x=t[breaks[i-1]:breaks[i]], y=flow_rate_mon.v[breaks[i-1]:breaks[i]]),
              np.trapz(x=t[breaks[i-1]:breaks[i]], y=flow_rate_ctabtpa.v[breaks[i-1]:breaks[i]]),
              np.trapz(x=t[breaks[i - 1]:breaks[i]], y=flow_rate_ctapy.v[breaks[i - 1]:breaks[i]]),
              np.trapz(x=t[breaks[i - 1]:breaks[i]], y=flow_rate_fl.v[breaks[i - 1]:breaks[i]])
              )

    data = np.column_stack((t, temp, light, flow_rate_mon.v, flow_rate_ctabtpa.v, flow_rate_ctapy.v, flow_rate_fl.v))
    # np.savetxt("DW2_10_profiles.csv", data, delimiter=",")
