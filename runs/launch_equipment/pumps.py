...
pump_one = chembot.equipment.pumps.SyringePumpHarvard(
    name=NamesPump.ONE,
    syringe=chembot.equipment.pumps.Syringe.get_syringe("norm_ject_5ml"),
    port="COM11",
    max_pull=12 * Unit.cm,
)
# pump_front.activate()
...
pump_five = chembot.equipment.pumps.SyringePumpHarvard(
    name=NamesPump.FIVE,
    syringe=chembot.equipment.pumps.Syringe.get_syringe("norm_ject_20ml"),
    port="COM21",
    max_pull=8 * Unit.cm,
)

with chembot.utils.EquipmentManager() as manager:
    manager.add([pump_one, pump_two, pump_three, pump_four, pump_five])
    manager.activate()
