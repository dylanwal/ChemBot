        ...
        while not self.event.is_set():
            ...
            if not self.queue.empty():
                ...
                if self.setpoint > self.max_temp:
                    exit("set point temperature set too high")
            ...
            if any(temp < 18 or temp > self.max_temp for temp in self.current_temp):
                ...
                if time() - error_timer > 10:
                    ...
                    self.event.set()
                    exit("max temperature hit or thermocouple signal lost")
            else:
...
def layout_1(options):
    ...
    # PID parameters
    temp_probe = 0  # the temperature probe for PID control
    PID_K = 0.00001
    PID_D = 500
    PID_I = 1
    heat_pad_pin = "D18"
...
if __name__ == '__main__':
    """
    Mode 1: Runs PID controller, set temperature can be changed on the fly, data will be saved to csv
